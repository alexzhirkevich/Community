...
@HiltViewModel
class EditProfileDescriptionViewModel @Inject constructor(
    ...
    override fun setText(text: String) = synchronized(mText) {
        if (text.length in 0..Config.DescriptionMaxLength)
            mText.tryEmit(text)
    }
        ...
        if (error.value is EditProfileError.None) {
            try {
                viewModelScope.launch(Dispatchers.IO) {
                    userProfileRepo.setDescription(usersRepo.currentUserId, text.value)
                    onSuccess()
                }
            } catch (t: CancellationException) {
            } catch (t: Throwable) {
                logger?.log(
                    "Failed to set profile description",
